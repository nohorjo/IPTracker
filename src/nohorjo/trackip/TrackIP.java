...
public class TrackIP extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (new HttpOperation().doGet("http://whatismyipaddress.com/ip/" + ip)
					.contains("<tr><th>Organization:</th><td>Facebook</td></tr>")) {
				resp.setContentType("text/html");
				resp.getWriter().write("<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>" + urlData[1]
						+ "</title></head><body></body></html>");
				return;
			}

			resp.setStatus(HttpServletResponse.SC_MOVED_TEMPORARILY);
			resp.setHeader("Location", urlData[0]);
			return;
		} catch (Exception e) {
		}
		...
		try {
			new IPDAO().recordAccess(ip, session.getId(), newCookie.getValue(), visitedBefore, urlid);
		} catch (Exception e) {
		...
		getServletContext()
				.getRequestDispatcher("/error#.html".replace("#", "" + new Random().nextInt(numberOfErrorPages + 1)))
				.forward(req, resp);
